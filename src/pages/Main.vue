<template>
  ...
  <div class="pa-2">
    ...
    <div v-if="userRole == 'faculty-affairs-office'">
      ...
      <div class="pb-2">
        كمكتب شؤون أعضاء هيئة تدريس من واجباتك مراجعة البيانات المتعلقة بطلب
        الترقية والمستندات المرفقة بالطلب و التأكد من صحتها.
      </div>

      <div v-if="user.currentDegree !== 'أستاذ'" class="pb-2">
        يمكنك أيضا تقديم الطلبات عن طريق هذا الحساب.
      </div>
    </div>

    <v-divider></v-divider>

    <div v-if="user.currentDegree !== 'أستاذ'">
      <div class="font-weight-bold pt-2">معلومات تهمك عند تصفح الطلب:</div>
      <ul>
        <li class="mr-5 py-1">
          يجب إكمال الطلبات عبر الإنترنت. يتم تمييز الأسئلة المطلوبة بعلامة
          النجمة
          <span style="color: red"> الحمراء (*).</span> يمكنك الوصول إلى الطلب
          الخاص بك وتغيير إجاباتك باستخدام البريد الجامعي وكلمة المرور في أي
          وقت.
        </li>
        <li class="mr-5 py-1">
          بينما يتم قبول عدد من إمتدادات الملفات، للحفاظ على التنسيق ولضمان
          تقديم الأحرف الخاصة بشكل صحيح، يوصى بشدة بتقديم المرفقات بتنسيق PDF.
        </li>
        <li class="mr-5 py-1">
          لا تحتاج إلى إكمال هذا الطلب في جلسة واحدة. لحفظ إجاباتك انقر على
          "حفظ" الموجود أسفل كل صفحة. ستتمكن من الوصول إلى أي عمل محفوظ خلال
          جلستك التالية.
        </li>
        ...
        <li class="mr-5 py-1">
          ...
          "مراجعة الطلب". بعد التقديم، لن تتمكن من إجراء تغييرات على طلبك الا
          باذن رئيس قسمك العلمي.
        </li>
      </ul>
    </div>
  </div>
